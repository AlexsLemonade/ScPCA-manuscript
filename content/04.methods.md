...
Solid tumor samples with both bulk and single-cell (or single-nuclei) RNA-seq data in the ScPCA Portal were identified for analysis, with multiplexed samples excluded (N=105).
Low-quality samples were identified based on visual inspection of quality control reports and removed (N=8), leaving a total of 97 samples across five ScPCA projects for analysis.
All analyses described here were conducted separately for each project.

Bulk counts matrices for all samples within each project were transformed and normalized with respect to library size using `DESeq2::rlog()` [@doi:10.1186/s13059-014-0550-8].
Pseudobulk counts were obtained by summing raw single-cell counts for each sample, and similarly transforming the resulting counts matrix for each project with `DESeq2::rlog()`.
...
To this end, we summarized model residuals within each project by taking the median residual for each gene across samples, then transforming these summarized residuals into Z-scores.
...
Gene sets with significant overrepresentation were defined as those with a false discovery rate corrected P-value ≤ 0.05 [@doi:10.1111/j.2517-6161.1995.tb02031.x].
