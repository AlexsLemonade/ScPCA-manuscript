...
The LCA was then used as the consensus cell type label if the following criteria was met, otherwise no consensus cell type was assigned:

1. The terms shared only 1 distinct LCA.
The only exception to this rule was if the terms shared 2 LCA and one of the identified LCA terms corresponded to `hematopoietic precursor cell`, in which case `hematopoietic precursor cell` was used as the consensus label. 

2. The LCA had fewer than 170 descendants, with the exception of `neuron` and `epithelial cell`.
