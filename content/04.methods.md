...
For this, we first identified cell types of interest as all possible consensus cell types for a given project.
We then established gene sets for each consensus cell type as the union from marker genes in the project's `CellAssign` marker gene reference.
Because the consensus cell types can encompass multiple cell types in the given marker gene reference, we defined each consensus cell type's gene set as the union of all gene sets for the constituent reference cell types.

For input to the overrepresentation analysis, we summarized model residuals by taking the median residual for each gene across samples within each project and then transformed these summarized residuals into Z-scores.
