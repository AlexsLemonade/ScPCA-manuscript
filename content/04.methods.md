...
Raw data (e.g., FASTQ files) are not available for download from the Portal due to the human origins of most samples, which may be subject to controlled-access restrictions. When raw or processed data are also deposited to external sources such as the Database of Genotypes and Phenotypes (dbGaP) or Gene Expression Omnibus (GEO), the project accession numbers are available from the Portal.
...
ScPCA documentation that describes the contents of downloads is available from the Portal are available at <https://scpca.readthedocs.io/en/stable/>.
