...
To identify cell types that might be overrrepresented in one modality compared to the other, we first identified gene sets associated with each cell type of interest, defined as each project's consensus cell types.
Because the consensus cell types can encompass multiple cell types from the `CellAssign` marker gene reference, we defined the consensus cell type gene sets as the union of all gene sets for the constituent cell types.

For input to the overrepresentation analysis, we summarized model residuals by taking the median residual for each gene across samples and then transforming these summarized residuals into Z-scores.
