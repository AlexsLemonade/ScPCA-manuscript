...
Marker genes used were from the list of Human cell markers on `CellMarker2.0` [@doi:10.1093/nar/gkac947] that are specific to a single cell-type with the exception of hematopoietic precursor cells, which express genes that are found in other cell types belonging to the hematopoietic cell lineage.
