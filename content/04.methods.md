...
However, we did not remove any cells based on ADT quality because this filtering would also remove those cells' RNA expression remove these cells from the `SingleCellExperiment` object altogether, including its RNA expression which may not have any quality concerns.
...
We performed normalization only on any cells that would be retained after ADT filtering with, and any cells that would be filtered out based on `DropletUtils::cleanTagCounts()` are assigned `NA`.
...
Note that we did not perform background substraction on the ADT counts, but we provide the ambient profile calculated with `DropletUtils::ambientProfileEmpty()` which users can employ for perform global denoising as needed.
