...
### Analysis of bulk data

#### Data preparation

Non-multiplexed solid tumor samples with both corresponding bulk and single-cell (or single-nuclei) data in the ScPCA Portal were identified for analysis (N=105).
Low-quality samples (N=8) based on visual inspection of quality control reports were then removed, leaving a total of 97 samples across five ScPCA projects for analysis.
All analyses described here were conducted separately for each project.

We obtained the bulk raw counts matrix for all samples of interest from Portal and transformed them with `DESeq2::rlog()` [@doi:10.1186/s13059-014-0550-8].
In addition, we established a matrix of pseudobulk counts by summing all raw single-cell counts in relevant processed objects from the ScPCA Portal, and we similarly transformed the resulting counts matrix with `DESeq2::rlog()`.
Genes which were not observed in either the bulk or pseudobulk raw counts matrices were filtered out before subsequent analysis.
...
We next performed an overrepresentation analysis on these model's residuals to test whether certain cell types were overrepresented in one modality compared to the other.
To this end, we summarized model residuals into per-gene quantities by taking the median residual across samples and then transforming these summarized residuals into Z-scores.
From each project's distribution of summarized residuals, we identified positive and negative outliers as those values greater than 2.5 or less than -2.5, respectively.
...
We prepared gene sets to represent consensus cell types for use in this ORA as follows.
We identified the consensus cell type categories that each fine-grained cell type in the project-specific `CellAssign` marker gene references contributed to.
We established gene lists for each consensus cell type as the union of those associated marker genes from the `CellAssign` reference.
...
To evaluate the significance of these odds ratios, we then performed permutation testing with 10,000 replicates.
P-values for both the bulk and pseudobulk enrichment directions were calculated as the fraction of permutations which yielded an odds ratio equal to or more extreme than the observed data odds ratio, with a lower bound of $9.99e-5$ (from $1/100001$).
We used the Benjamini-Hotchberg procedure [@doi:10.1111/j.2517-6161.1995.tb02031.x] to control the false discovery rate, and gene sets with significant overrepresentation were identified having a corrected $P-value <= 0.05$.
