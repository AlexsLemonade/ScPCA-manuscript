...
Cell type labels obtained from `SingleR` and `CellAssign` were then used to assign an ontology-aware consesnsus cell type label. 
...
The cell types available in the `BlueprintEncodeData` reference used with `SingleR` already contained Cell Ontology terms, so we did not need to assign these. 

We then created a reference table that contains all possible combinations of cell types assigned using `SingleR` and `CellAssign` and identifed the latest common ancestor (LCA) [@url:https://rdrr.io/bioc/ontoProc/man/findCommonAncestors.html] between the two cell type terms. 
The LCA was then used as the consensus cell type assignment as long as the LCA met the following set of criteria. 
If the terms shared more than 1 LCA, then no consensus cell type was assigned. 
The only exception to this rule was if one of the LCA terms corresponded to `hematopoietic precursor cell`. 
If this was the case then the other LCA terms were removed and `hematopoietic precursor cell` was used as the consensus label. 
If the LCA had greater than 170 descendants, then no consensus cell type was assigned, with a few exceptions. 
When the LCA was `neuron`, then `neuron` was used as the consensus cell type label. 
When the LCA was `epithelial cell` and the annotation from `BlueprintEncodeData` was `Epithelial cells`, then `epithelial cell` was used as the consensus label. 
If the LCA was `bone cell`, `lining cell`, `blood cell`, `progenitor cell`, or `supporting cell`, no consensus label was defined as we thought these terms were not specific enough to define a cell type. 
