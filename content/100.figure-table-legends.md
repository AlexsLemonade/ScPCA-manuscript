...
The y-axis displays the broad consensus cell type, with the total number of cells across libraries in parentheses.
The x-axis displays marker genes, determined by `CellMarker 2.0` [@doi:10.1093/nar/gkac947], used for consensus cell type validation for each cell type in the top annotation bar.
...
The y-axis displays the consensus cell type, with the total number of cells in parentheses.
The x-axis displays marker genes, determined by `CellMarker 2.0` [@doi:10.1093/nar/gkac947], used for consensus cell type validation for each cell type in the top annotation bar.
...
Each cell in `SCPCL000130` is represented by two adjacent rows, the first indicating presence or absence of copy-number gain and the second indicating presence or absence of copy-number loss.
...
D. Example ridge plot (`SCPCL000130`) as shown in the summary QC report showing per-cell total CNV distributions across the top seven consensus cell type annotations.
...
Samples with RNA-seq for both bulk and single-cell/nuclei modalities, excluding multiplexed samples, from ScPCA projects of brain and CNS tumors are shown, with the number of samples considered shown in parentheses.
...
Odds ratios represent the odds that marker genes for the given cell type were overrepresented in bulk RNA-seq compared to single-cell/nuclei RNA-seq, relative to other genes.
...
References were built using all cell types from a specified set of organs in `PanglaoDB`'s marker gene list.

`scpca_project_id`: ScPCA project unique identifier.
`Diagnoses`: Full set of diagnoses for all samples in the project.
`ScPCA reference name`: Custom reference name.
`PanglaoDB organs included in reference`: A list of all organs included in the reference corresponding to organs listed in `PanglaoDB`.
...
A grid of UMAPs with each library in the merged object is shown.
...
This UMAP shows the top three T cell types for each cell type assignment, with remaining T cell types combined in "other T cells."
...
Expression is shown for each broad cell type annotation, each of which represents a collection of similar consensus cell type annotations.
The y-axis displays the broad consensus cell type observed across libraries, with the total number of cells in parentheses.
The x-axis displays marker genes, determined by `CellMarker 2.0` [@doi:10.1093/nar/gkac947], used for consensus cell type validation for each cell type in the top annotation bar.
...
All samples shown here are single-nuclei, with the number of samples considered shown in parentheses.
