...
The heatmap is grouped by chromosome arm and OpenScPCA Project cell type annotation, where "normal" cells comprise all characterized non-maligant cells.
This library exhibits strong signatures of canonical Neuroblastoma alterations including `1p` loss, `11q` loss, and `17q` gain. 

D. Example ridgeplot as shown in the summary QC report showing per-cell total CNV distributions across the top seven consensus cell type annotations.
