...
A. UMAP highlighting cell type annotations made with the OpenScPCA Project, collapsed into broad annotation groups, for all Neuroblastoma libraries (N = 42) in the ScPCA Project `SCPCP000004`. 
...
B. UMAP highlighting total per-cell CNV events as calculated from `InferCNV` [@url:https://github.com/broadinstitute/inferCNV] for all Neuroblastoma libraries (N = 42) in the ScPCA Project `SCPCP000004`. 
The UMAP was constructed from the merged `SCPCP000004` object such that all libraries contribute an equal weight, but no batch correction was performed.

C. Heatmap displaying per-cell CNV events across chromosomes with canonical Neuroblastoma alterations [@doi:10.1038/nrdp.2016.78; 10.1016/j.celrep.2024.114804; 10.1158/2159-8290.CD-14-0622] for a single library, `SCPCL000130`, as inferred with InferCNV` [@url:https://github.com/broadinstitute/inferCNV].
The heatmap is separated into groups based on the OpenScPCA Project cell type annotation, where "normal" cells comprise all characterized non-maligant cells.
This library exhibits strong signatures of canonical Neuroblastoma alterations including `1p` loss, `11q` loss, and `17q` gain. 

D. Example ridgeplot as shown in the summary QC report showing the distributions of per-cell total CNV inferred with `InferCNV` [@url:https://github.com/broadinstitute/inferCNV] across the top seven consensus cell type annotations.
All additional consensus cell types are shown with the "all remaining cell types" category.
The ridgeplot shown is for library `SCPCL000130`. 
