...
C. Example heatmap as shown in the cell type summary report comparing the consensus cell type annotations to automated annotations made with `SingleR`, `CellAssign`, and `SCimilarity`. 
...
For this figure specifically, the heatmap shows only the top seven cell type annotations with at least three cells, but the heatmap in the cell type summary report shows all cell types.
<br><br>
