...
B. UMAP highlighting total per-cell CNV events as calculated from `InferCNV` [@url:https://github.com/broadinstitute/inferCNV]  for all libraries in the neuroblastoma-only ScPCA Project `SCPCP000004` (N = 42).
The UMAP was constructed from the merged `SCPCP000004` object such that all libraries contribute an equal weight, but no batch correction was performed.
