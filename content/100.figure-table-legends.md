...
A. Dot plot of marker gene expression across broad consensus cell type annotation groups, specifically considering ScPCA libraries for brain and CNS tumors.
...
B. Barplot showing the percentage of each broad consensus cell type annotation across libraries for brain and CNS tumors, separated into high-grade (left panel) and low-grade (right panel) glioma diagnoses.

C. Barplot showing the percentages of different consensus cell type immune cells across libraries for brain and CNS tumors, separated into high-grade (left panel) and low-grade (right panel) glioma diagnoses.
...
A. Scatter plots colored by point density of `DESeq2`-transformed and normalized bulk expression across pseudobulk expression.
Samples with both bulk and single-cell/nuclei modalities from ScPCA projects comprising brain and central nervous system tumors are shown, with the number of samples per panel shown in parentheses.
...
Each odds ratio represents the odds that marker genes for the given cell type were overrepresented in the bulk modality vs. the pseudobulk modality, relative to other genes.
...
B. This example heatmap from the cell type summary report compares submitter-provided annotations to annotations with `SingleR` and `CellAssign`, shown for library `SCPCL000498` [@doi:10.1016/j.devcel.2022.04.003].
...
<!-- Figure S6 -->
![**Consensus cell type annotations.**](https://raw.githubusercontent.com/AlexsLemonade/scpca-paper-figures/main/figures/compiled_figures/pngs/figure_s6.png?sanitize=true){#fig:figS6 tag="S6" width="9in"}

Panels show dot plots of marker gene expression across broad consensus cell type annotation groups, specifically considering cells across ScPCA diagnosis groups (A.) Leukemia, (B.) Sarcoma, and (C.) Other solid tumors.
...
Panels shown barplots of the percentage of each broad consensus cell type annotation across libraries for high-grade (left panel) and low-grade (right panel) glioma samples in the ScPCA Portal.

C. Barplot showing the percentages of different consensus cell type immune cells across libraries for high-grade (left panel) and low-grade (right panel) glioma samples in the ScPCA Portal.
Only libraries comprised of at least 1\% immune cells, based on consensus cell type annotations, are shown.
Specific consensus cell types for myeloid and leukocyte immune cells are shown, with all other consensus immune cell types included "other."
Notably, granulocytes are also included in "other" because only 1 granulocyte was present in all libraries shown (specifically, `SCPCL000793`).
...
A. Scatter plots colored by point density of `DESeq2`-transformed and normalized bulk expression across pseudobulk expression.
Samples with both bulk and single-cell/nuclei modalities, excluding multiplexed samples, are shown, with the number of samples per panel shown in parentheses.
The regression line is also shown for each project.
