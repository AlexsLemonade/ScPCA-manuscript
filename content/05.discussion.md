...
We then used the correspondence among these three methods to derive ontology-aware consensus cell type labels.
The consensus cell type labels therefore provide a consistent labeling scheme across samples and may be beneficial for annotating populations of normal cells that may be present in tumor samples.
A limitation of our annotation approach is that neither the references we used for `SingleR` or `CellAssign` nor the `SCimilarity` foundation model consider tumor cells; therefore, tumor cells are likely poorly assigned.
To help circumvent this limitation, we have additionally provided CNV estimates, as inferred with `InferCNV`, for the majority of libraries in the ScPCA Portal. 
The combined information from consensus cell type annotations and CNV estimates may provide an avenue for researchers to identify and interrogate malignant cells, in particular for diagnoses where genomic alterations are common such as neuroblastoma (Figure {@fig:fig5D}) or osteosarcoma.
...
Unlike annotations made within the `scpca-nf` pipeline, these labels do include formal identification of normal vs. malignant cell types. 
Moving forward, we will continue to expand the set of projects on the Portal that include OpenScPCA cell type annotations. 
These ongoing efforts will continue to enrich the Portal with high-quality, well-documented annotations that support scientific discovery and reuse by the research community.
