...
Several projects in the ScPCA Portal quantified many samples' expression both at the bulk and single-cell level.
Previous research has suggested that, compared to bulk RNA-seq, single-cell RNA-seq may fail to capture certain cell types [@doi:10.1038/s41587-020-0465-8] due, for example, to technical aspects of library preparation.
We therefore sought to ask whether we could identify differences in cell type composition between these two modalities.
For this analysis, we specifically focused on ScPCA projects with solid tumors, considering only non-multiplexed samples with both sequencing modalities and excluding low-quality single-cell libraries.
...
As described in Materials and Methods, we derived pseudobulk expression matrices from the single-nuclei libraries, and we compared their expression to bulk using a series of linear models (one per ScPCA project) predicting bulk from pseudobulk expression, with a random effect controlling for sample (Figure {@fig:fig6}A, Figure {@fig:figS8}A).
We observed an expected positive relationship between bulk and pseudobulk expression across all projects.

We next performed an overrepresentation analysis to test if cell type marker genes for our consensus cell types were enriched in either modality, which may in turn indicate that certain cell types were more prevalently detected by either modality.
To this end, we calculated the per-gene median of each project's model residuals and identified outliers, where positive outliers represent genes that are enriched in bulk RNA-seq, and negative outliers represent genes that are enriched in single-cell/nuclei RNA-seq.
For each modality, we calculated the odds ratio as: the odds a cell type marker gene is present in the given modality's outliers compared to other genes.
Following permutation testing and P-value correction to control the FDR at $5%$, we indeed found that several cell type marker gene sets were overrepresented in the bulk modality, but never in the pseudobulk modality (Figure {@fig:fig6}B, Figure {@fig:figS8}B).

In brain and CNS tumors, we nearly exclusively identified stromal (e.g., endotheliel and extracellular matrix secreting cells) and/or neuronal cell types (e.g., glial cells and astrocytes), all of which are known to be prevalent non-immune cells in glioma tumor microenvironments [@doi:10.3389/fimmu.2023.1227126; @doi:10.3389/fphar.2024.1355242], as overrepresented in bulk as opposed to immune cell types (Figure {@fig:fig6}B).
...
Given that our consensus cell type analysis identified a variety of immune cells from high- and low-grade gliomas (Figure {@fig:fig5}), this analysis suggests that non-immune cells may have been lost during single-cell library preparation.
Indeed, several of these overrepresented bulk cell types for `SCPCP000001` and `SCPCP000002` were not among the single-nuclei consensus cell types themselves (`SCPCP000001`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "glial cell", "pericyte"; `SCPCP000002`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "microvascular endothelial cell"), further emphasizing loss of these cell types in the single-nuclei data.
By contrast, we uncovered a variety of both immnune and non-immune cel types overrepresented in bulk RNA-seq `SCPCP000017` (osteosarcoma; Figure {@fig:figS8}B), which may reflect inherent challenges in dissociating bone tissuse [@doi: https://doi.org/10.1186/s12885-023-10977-1].
Taken together, these results show that, while bulk and single-cell/nuclei expression is indeed highly correlated, differences in cell type composition may still be present between modalities potentially driven by cell-type--specific loss in single-cell experiments.
