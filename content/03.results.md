...
Note that there are seven samples in the Portal without single-cell/nuclei RNA sequencing data that have only bulk RNA-seq and/or spatial transcriptomics data.
...
We validated consensus cell types using the expression of cell-type-specific marker genes across all cells to validate the assignments (Figure {@fig:fig4}A, Figure {@fig:figS5}), observing broad correspondance between consensus cell type labels and expected marker gene expression.
