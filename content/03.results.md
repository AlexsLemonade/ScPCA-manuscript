...
We specifically focused on ScPCA projects with solid tumors, considering only samples with both sequencing modalitiesm, excluding low-quality libraries and multiplexed samples.
...
In brain and CNS tumors, the marker gene sets overrepresented in bulk RNA-seq expression corresponded nearly exclusively to stromal (e.g., endothelial and extracellular matrix secreting cells) and/or neuronal cell types (e.g., glial cells and astrocytes), all of which are known to be prevalent non-immune cells in glioma tumor microenvironments [@doi:10.3389/fimmu.2023.1227126; @doi:10.3389/fphar.2024.1355242] (Figure {@fig:fig6}B).
