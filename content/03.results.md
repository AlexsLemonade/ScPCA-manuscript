...
> sum(!(sce$openscpca_celltype_annotation %in% c("Unknown", "openscpca-excluded"))) / ncol(sce)
[1] 0.9095714

> sum(sce$consensus_celltype_annotation != "Unknown") / ncol(sce)
[1] 0.4342133
-->
We plan to continue expanding the number of projects on the Portal with manually-curated annotations from the OpenScPCA project. 
...
In addition, we included a step in the `scpca-nf` pipeline to run `InferCNV` [@url:https://github.com/broadinstitute/inferCNV] to quantify copy-number alterations across libraries, specifically for libraries with at least 100 normal cells (as determined by consensus cell types) to use as the baseline for CNV inference (Figure {@fig:fig2}A).
...
We see similar patterns for the ganglioglioma library `SCPCL000049` (Figure {@fig:figS4}B-C), where consensus T cells have low total CNV values, while other cell types including oligodrendrocyte precursor cells, neuron associated cells, and Unknown cells have much higher total CNV values. 
These observations provide confirmation that the identified consensus T cells (Figure {@fig:figS4}A) are indeed not likely to be malignant and provide evidence for which cells in the library may be malignant. 
