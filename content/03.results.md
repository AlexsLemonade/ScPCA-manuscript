...
When comparing annotations obtained from `CellAssign` to `SingleR` annotations and submitter-provided annotations, we noticed the labels for non-tumor cells are similar between `CellAssign`, `SingleR`, and submitter annotations, while the tumor cells were not assigned using `CellAssign` (Figure {@fig:figS5}B).
...
In addition to the cell type annotations assigned by `SingleR` and `CellAssign`, we assigned a consensus cell type label based on the agreement between the two automated methods. 
These two automated methods use different references and different approaches to label cells so we expect that cells with the same, or similar, cell type label using both methods are likely to be more accurate. 
To account for different levels of granularity in the cell types included in each of the reference datasets, we used an ontology-based approach to assign the consensus cell type. 
In particular, the consensus cell type annotation is equivalent to the latest common ancestor (LCA) [@url:https://rdrr.io/bioc/ontoProc/man/findCommonAncestors.html] shared between the two cell types. 
<!--Not sure if we need to get into details here since we have the number in the methods, but I think it's probably helpful to mention that we wanted some specificity in our labels and didn't just blindly use the LCA-->
If the LCA had more than 170 descendents, with some exceptions, then we did not choose to assign a consensus cell type and instead labeled these as unknown. 
The cutoff of 170 descendants was chosen to eliminate any LCAs that were non-specific cell ontology terms, like lymphocyte, while keeping specific terms like T cell and B cell. 
<!--TODO: If this stays the way it is, then we should probably switch the numbering for these figures? -->
A summary of all the consensus cell types assigned in all ScPCA samples can be found in {@fig:fig5}B and {@fig:figS7}. 
After assigning all consensus cell types, we looked at expression of cell-type specific marker genes across all cells {@fig:fig5}A and {@fig:figS6} to validate our assignments. 
<!--TODO: Add in more information about how we defined marker genes to the methods sections?-->

Assigning the consensus cell type creates a common annotation for all 700 samples in the ScPCA Portal, making it easy for users to perform downstream analyses across multiple samples and projects. 
This can be particularly useful when looking at samples from the same disease type that are present in multiple projects and submitted by different investigators. 
An example of this is the group of samples belonging to the Portal that are classified as High-grade (HGG) and Low-grade gliomas (LGG). 
These samples belong to 6 different projects on the Portal and were submitted by 4 different investigators. 
{@fig:fig5}B shows the composition of all cell types observed across all High-grade and Low-grade glioma samples. 

Previous publications have highlighted that the immune microenvironment of gliomas are mostly comprised of myeloid cells, such as microglia and glioma-associated macrophages, with smaller percentages of lymphocytes, such as T cells and natural killer cells [@doi:10.1038/s41698-024-00717-4;@doi:10.1093/noajnl/vdad009].
Consistent with previous literature, zooming in on the immune infiltrate in these tumors reveals that the majority of cells are classified as either a myeloid cell type or T cell type, with quite a bit of heterogeneity, even within HGG and LGG.  
