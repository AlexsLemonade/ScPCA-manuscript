...
The most common sample diagnosis on the Portal is leukemia, followed by sarcoma and soft tissue tumors, brain and central nervous system tumors, and various other solid tumors.
...
Some samples have additional data, such as CITE-seq quantification of cell-surface protein levels with antibody-derived tags (ADTs; n = 95) [@doi:10.1038/nmeth.4380], or hashtag oligonucleotide (HTO) quantification for multiplexed samples (n = 35) [@doi:10.1186/s13059-018-1603-1].
In some cases, multiple libraries from the same sample were created for additional assays, either for bulk RNA-seq (n = 182) or spatial transcriptomics (n = 41). <!-- Note context for these values: https://github.com/AlexsLemonade/ScPCA-manuscript/issues/245 -->
Figure {@fig:fig1}B shows a full summary of sequencing modalities across samples, with additional details available in Table S1.
Note that the Portal does not distribute raw FASTQ files, but links to raw data sources in external repositories, e.g., the Database of Genotypes and Phenotypes (dbGaP) [@doi:10.1038/ng1007-1181; @doi:10.1093/nar/gkt1211], are provided when available.

Samples on the Portal are organized by project, defined as a collection of similar samples from an individual lab.
Users can filter projects based on diagnosis, modalities (e.g., CITE-seq, bulk RNA-seq), 10x Genomics kit version (e.g., 10Xv2, 10Xv3), and whether a project includes samples from patient-derived xenografts or cell lines.
...
Cell type reports for these projects include additional tables and plots with these annotations, including comparisons to automated cell typing results.
...
Consensus cell types are available in all processed objects on the Portal.
...
Library-specific versions of Figures {@fig:fig4}A and {@fig:fig3}C are included in QC reports, allowing users to assess the reliability of consensus annotations and compare labels across methods.
...
Note that Figure {@fig:figS6} summarizes consensus cell types across other ScPCA samples.
...
Figure {@fig:fig4}D shows the expression of cell-type specific markers for more granular immune cell types in brain and CNS tumors, validating the assignment of various immune cell types within the assigned consensus cell types.
...
Thus far, we have added OpenScPCA cell type annotations for two projects, `SCPCP000004` (neuroblastoma) and `SCPCP000015` (Ewing sarcoma), to the Portal.
...
Unlike the consensus cell types, the OpenScPCA annotations distinguish normal and malignant cells and contain fewer uncharacterized cells.
Summary cell type reports for projects with OpenScPCA annotations include comparisons between `scpca-nf` and OpenScPCA annotations.

In addition, `scpca-nf` applies `InferCNV` with the `i6` hidden Markov model [@url:https://github.com/broadinstitute/inferCNV] to estimate copy-number alterations (Figure {@fig:fig2}A) when enough normal cells are present in a library to serve as a reference (see Methods).
The CNV estimates provide a proxy for a cell's malignant status; cells with high levels of CNV are more likely to be tumor cells.
...
In one example neuroblastoma library, we observe canonical neuroblastoma CNV events including `1q` loss, `11q` gain, and `17p` loss [@doi:10.1038/nrdp.2016.78; @doi:10.1016/j.celrep.2024.114804; @doi:10.1158/2159-8290.CD-14-0622] within malignant cells (Figure {@fig:fig5}C).
...
We see similar patterns in a ganglioglioma library (Figure {@fig:figS4}B-C); consensus immune cell types have low CNV values, while other cell types including oligodendrocyte precursor cells, neuron associated cells, and unknown cells have higher CNV values.

### Analysis of bulk RNA-seq

Several projects in the ScPCA Portal contain bulk RNA-seq data in addition to sc/snRNA-seq data.
...
We therefore asked whether biological signal differed between modalities in ways suggesting distinct cell type distributions by interrogating ScPCA solid tumor projects with both sequencing modalities.
...
As described in Methods, we derived pseudobulk expression matrices for each single-cell/nuclei library and compared the pseudobulk expression to bulk using a series of linear models with a random effect controlling for sample (Figure {@fig:fig6}A, Figure {@fig:figS7}A).
...
We found several cell type marker gene sets with higher, but never lower, bulk RNA-seq expression than expected (Figure {@fig:fig6}B, Figure {@fig:figS7}B).
