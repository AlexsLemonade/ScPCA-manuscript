...
Out of the over 700 samples, 95 have associated CITE-seq data, and 35 have associated data with cell hashing.
...
Specifically, 176 samples on the Portal were sequenced using bulk RNA-seq and 38 samples were sequenced using spatial transcriptomics.
...
The project card also indicates the type(s) of sequencing performed, including the 10x Genomics kit version, the suspension type (cell or nucleus), and if additional sequencing is present, like bulk RNA-seq or cell hashing.
...
No further processing besides merging object is performed; batch-correction is not performed, and integrated data is not provided.
...
The classification model and processed `SingleCellExperiment` were inputted to `SingleR::classifySingleR()`, resulting in annotations for all cells and an associated matrix with per-cell scores for all possible cell type labels.
...
The only exception to this was monocyte marker genes being overrepresented in bulk RNA-seq expression for `SCPCP000009` (brain and CNS tumors), which was sequenced at the single-nuclei level while projects `SCPCP000001` (high-grade gliomas) and `SCPCP000002` (low-grade gliomas) were sequenced at the single-cell level.
