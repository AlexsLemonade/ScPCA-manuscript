...
Therefore, we provide a single merged object for each ScPCA project containing all raw and normalized gene expression data and metadata for all single-cell and single-nuclei RNA-seq libraries (with some exceptions as described in the Methods).
...
When downloading the merged object for an ScPCA project, the download will include a single `.rds` or `.h5ad` file, a summary report for the merged object, and the individual QC and cell type reports for each individual library (Figure {@fig:fig2}I).
