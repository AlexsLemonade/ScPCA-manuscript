...
The workflow outputs a `SingleCellExperiment` object (saved as an `.rds` file) containing the fully processed results, including the dimension reduction results and cell typ annotations, as well as objects containing the unfiltered and the empty drop filtered gene-by-cell matrices
...
In the faceted plot, each panel highlights a different library included in the merged object, with cells from the specified library shown in color, while all other cells are gray.
<!-- JAS: I would probably cut the previous sentence entirely; the level of detail seems excessive for the main text -->
An example of this plot, showing a subset of libraries from an ScPCA project, is available in Figure {@fig:fig3}D.
...
Using this measure, we found that the `BlueprintEncodeData` reference [@doi:10.3324/haematol.2013.094243; @doi:10.1038/nature11247], which includes a variety of normal cell types, tended to perform similarly to or better than other references when applied to samples from different disease types (Figure {@fig:figS4}).
...
For each ScPCA project, we used the organ-specific marker gene matrix relevant to the disease and tissue type from which the sample was obtained (e.g., for brain tumors, we used a brain-specific marker gene matrix with all brain and immune cell types).
If `CellAssign` cannot find a likely cell type from the marker gene matrix, it does not assign a cell type.
Because we annotate cells from tumor samples using references containing only normal cells, we anticipate that many cells, particularly the tumor cells, will not have an exact match; reporting this to the end user is valuable.
...
The `SingleR` score matrix and the assigned cell types are added to the processed objects output by `scpca-nf`.
...
This index is calculated between pairs of cell type labels from each method and ranges from 0-1, with a value close to 1 indicating high agreement and a high proportion of overlapping cells and values close to 0 indicating a low proportion of overlapping cells.
...
The addition of consensus cell type labels provides a harmonized cell type annotation for all samples in the ScPCA Portal, facilitating downstream analyses across multiple samples.
