...
`SingleR` and `CellAssign` use different references and distinct computational approaches to label cells so we expect that cells with the same, or similar, cell type label using both methods are likely to be more accurate. 
...
After assigning all consensus cell types, we looked at the expression of cell-type specific marker genes across all cells to validate our assignments ({@fig:fig5}A, {@fig:figS6}). 

Addition of the consensus cell type label provides a harmonized cell type annotation for all 700 samples in the ScPCA Portal, making it easy for users to perform downstream analyses across multiple samples. 
This can be particularly useful when looking at samples from multiple projects and submitted by different investigators as the availability of uniformly processed gene expression data with provided consensus annotations makes it easy to perform analysis and make conclusions across a large set of samples. 
As an example of this, we show the distribution of cell types observed in all high-grade and low-grade glioma samples in {@fig:fig5}B, which originate from six different projects and four different investigators. 
Here we are able to identify similar cell types across all glioma samples but can see that the composition of cell types present in each sample is heterogeneous.
...
Zooming in on the immune infiltrate in glioma samples reveals that the majority of immune cells in ScPCA samples are indeed classified as either myeloid cell types or T cell types, although there is notable heterogeneity even within HGG and LGG subtypes. 
A summary of all the consensus cell types observed in all other ScPCA samples can be found in {@fig:figS7}. 
