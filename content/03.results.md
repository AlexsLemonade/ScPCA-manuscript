...
These two automated methods use different references and distinct computational approaches to label cells so we expect that cells with the same, or similar, cell type label using both methods are likely to be more accurate. 
To account for different levels of granularity in reference datasets, we employed an ontology-based approach to assign the consensus cell type. 
...
To ensure specificity in our consensus labels, we imposed a threshold on the LCA’s specificity. 
If the LCA had more than 170 descendant terms—indicating a broad or non-specific classification—we did not assign a consensus label and instead marked these cells as "unknown." 
...
Marker genes were defined based on the CellMarker2.0 database, selecting genes specific to a single cell type, with the exception of hematopoietic precursor cells, which exhibit expression of multiple lineage-specific genes.

Assigning the consensus cell type creates a common annotation for all 700 samples in the ScPCA Portal, making it easy for users to perform downstream analyses across multiple samples. 
...
For example, high-grade (HGG) and low-grade glioma (LGG) samples within the Portal originate from six different projects submitted by four independent investigators. 
The distribution of cell types across all glioma samples is shown in {@fig:fig5}B.
...
Consistent with these findings, zooming in on the immune infiltrate in glioma samples confirms that the majority of immune cells are classified as either myeloid cells or T cells, though there is notable heterogeneity even within HGG and LGG subtypes.
