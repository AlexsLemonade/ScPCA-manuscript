...
When downloading a complete project, users can either download individual files for each sample (default) or one file containing the gene expression data and metadata for all project samples in the project as a merged object; the file structure for downloaded projects is shown in Figure {@fig:fig2}H-I.
...
Values of 1 indicate bhnvb a high proportion of overlapping cells, which may indicate higher confidence predictions.
