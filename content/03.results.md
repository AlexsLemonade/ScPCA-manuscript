...
Previous research has suggested that, compared to bulk RNA-seq, and single-cell scRNA-seq may fail to capture certain cell types [@doi: @10.1038/s41587-020-0465-8] due, for example, to technical aspects of library preparation.
...
In total, we analyzed 97 samples across five projects (`SCPCP00001`, `SCPCP000002`, `SCPCP000006`, `SCPCP000009`, and `SCPCP000017`), all of which were single-nuclei experiments.
...
We next performed an overrepresentation analysis to test if cell type marker genes were enriched either modality.
We calculated the per-gene median of each project's model residuals and identified outliers in the bulk ($>2.5\sigma$) or pseudobulk direction ($<-2.5\sigma$).
Using a set of cell type marker genes for each consensus cell type, we calculated the odds ratio for each modality as: the odds a cell type marker gene is present in the given modality's outliers compared to other genes.
...
This in turn suggests that those cell types may be more prevalently detected in bulk RNA-Seq.
Most notably, for projects `SCPCP000001` (high-grade gliomas) and `SCPCP000002` (low-grade gliomas), several of these overrepresented cell types were not among the single-nuclei consensus cell types themselves (`SCPCP000001`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "glial cell", "pericyte"; `SCPCP000002`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "microvascular endothelial cell"), many of which are expected to be found in the glioma tumor microenvironment [doi: @10.3389/fimmu.2023.1227126].
Taken together, these results suggest that, while bulk and single-nuclei expression are indeed highly correlated, these modalities indeed detect different biological signal which may be indicative of distinct cell type distributions.
