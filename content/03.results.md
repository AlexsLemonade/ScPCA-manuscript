...
As described in Materials and Methods, we derived pseudobulk expression matrices from the single-nuclei libraries, and we compared their expression to bulk using a series of linear models (one per ScPCA project) predicting bulk from pseudobulk expression, with a random effect controlling for library (Figure {@fig:fig6}A, Figure {@fig:figS8}A).
We observed an expected positive relationship between bylk and pseudobulk expression across all projects.
...
Most notably, for projects `SCPCP000001` and `SCPCP000002`, several of these overrepresented cell types were not among the single-nuclei consensus cell types themselves (`SCPCP000001`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "glial cell", "pericyte"; `SCPCP000002`: "blood vessel endothelial cell", "extracellular matrix secreting cell", "microvascular endothelial cell").
